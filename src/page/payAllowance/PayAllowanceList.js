...
import { AllowanceListItem } from "./component/AllowanceListItem"
...
@observer
class PayAllowanceList extends React.Component {
    @observable data = {
        userid: '',
        type: 'ListCurrentPayment',
        params: {
            Name: '',
            TermType: 0,
            Dept: '',
    ...
    handleClickAdd = () => {
        window.location.href = '/home/payAllowanceAdd?' + BaseComm.UrlVersion();
    };

    payAllowanceList = () => {
        if (!this.data.isShowFilter && !this.data.payAllowanceList) {
            this.data.isShowFilter = true;
            this.data.tipsText = '请稍等...';
            this.data.isShowTips = true;
        }
        let self = this.data;
        let data = self.payAllowanceList;
        self.noData = false;
        axios({
            ...
            if (data.length > 0) {
                if (res.data.length > 0) {
                    self.payAllowanceList = self.payAllowanceList.concat(res.data);
                ...
                if (res.data.length > 0) {
                    ...
                    if (res.data.length < 10) {
                        self.isShowLoadMore = true;
                        self.moreText = CommonConfig.loadFinishTxt;
                        sessionStorage.setItem("payAllowanceList", JSON.stringify(self.payAllowanceList));
                    } else {
                        self.isShowLoadMore = true;
                        self.moreText = CommonConfig.loadMoreTxt;
                        sessionStorage.setItem("payAllowanceList", JSON.stringify(self.payAllowanceList));
            ...
            <div className="wrapper">
                <div className="wrapper-detail">
                    ...
                    <div className="list-content" ref={content => { this.content = content }}>
                        {this.data.payAllowanceList.map(item => (
                            <AllowanceListItem listData={item}
                                key={item.PFI_ID}
...
export default PayAllowanceList;