...
import { AllocationListItem } from "./component/AllocationListItem";
import AllocationListFilter from "./component/AllocationListFilter";
import BaseComm from "../commonInterface/BaseComm";

@observer
class PayAllocationList extends React.Component {
    @observable data = {
        userid: '',
        type: 'ListAllocation',
        params: {
    ...
    componentDidMount() {
        ...
        this.data.resData = sessionStorage.getItem("payAllocationList");
        if (this.data.resData !== null && this.data.resData !== undefined) {
            this.data.payAllocationList = JSON.parse(this.data.resData);
    ...
    handleClickAdd = () => {
        window.location.href = '/home/payAllocationAdd?' + BaseComm.UrlVersion();
    };
    ...
    payAllocationList = () => {
        if (!this.data.isShowFilter && !this.data.payAllocationList) {
            this.data.isShowFilter = true;
            this.data.tipsText = '请稍等...';
            this.data.isShowTips = true;
        }
        ...
        axios({
            ...
            if (this.data.isShowTips) {
                self.payAllocationList = [];
            }
            this.data.isShowFilter = false;
            this.data.isShowTips = false;
            this.data.tipsText = '';
            if (self.payAllocationList.length > 0) {
                if (res.data.length > 0) {
                    self.payAllocationList = self.payAllocationList.concat(res.data);
                ...
                if (res.data.length > 0) {
                    ...
                    if (res.data.length < 10) {
                        self.isShowLoadMore = true;
                        self.moreText = CommonConfig.loadFinishTxt;
                        sessionStorage.setItem("payAllocationList", JSON.stringify(self.payAllocationList));
                    } else {
                        self.isShowLoadMore = true;
                        self.moreText = CommonConfig.loadMoreTxt;
                        sessionStorage.setItem("payAllocationList", JSON.stringify(self.payAllocationList));
        ...
        return (
            <div className="wrapper">
                <div className="wrapper-detail">
                    ...
                    <HeadTitle titleTxt={this.data.title} showAddBtn={true}
                    ...
                    <div className="list-content" ref={content => { this.content = content }}>
                        {this.data.payAllocationList.map((item, index) => (
                            <AllocationListItem listData={item}
                                key={index}
...
export default PayAllocationList;
